...
Once a drone is assigned its specific cluster of deliveries, it needs to know the most efficient order to visit them (The Traveling Salesperson Problem). The simulation invokes Swarm Intelligence via an ACO algorithm. It simulates artificial 'ants' dropping pheromones to find the shortest continuous physical sequence between the delivery nodes.
