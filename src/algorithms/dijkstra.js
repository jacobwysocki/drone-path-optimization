...
const getNewGridWithNodesFromShortestPathDisappear = (grid, row, col) => {
...
export function getNodesInShortestPathOrder(startNode, finishNode, grid) {
    ...
    const t = shortestArray.map(([row, col]) => {
        return someFunction(row, col); // Assign the variables to some function and return the result
    });

    console.log(t[0]);
    const newGrid = getNewGridWithNodesFromShortestPathDisappear(grid, t[0][0], t[0][1]);
    //console.log(newGrid);
